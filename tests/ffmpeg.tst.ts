...
describe('FFMPEG COMMAND GENERATION TEST', function() {
  it('Filter #1', () => {
    ...
    expect(output.compile().join(' ')).toEqual('ffmpeg -i input.mp4 -filter_complex [0]hflip=x=10:y=20[s0] -map [s0] output.mp4');
  });
  ...
  it('Filter #2', () => {
    ...
    const output = filter.output('output.mp4');
    expect(output.compile().join(' ')).toEqual('ffmpeg -i hi.mp4 -i hi.png -filter_complex [0][1]overlay[s0] -map [s0] output.mp4');
  });

  it('CONCAT #1', () => {
    const concat = ffmpeg.input('list.txt', {f: 'concat', safe: '0', c: 'copy'});
    const output = concat.output('output.mp4');
    expect(output.compile('./ffmpeg', true).join(' ')).toEqual('./ffmpeg -f concat -safe 0 -c copy -i list.txt output.mp4 -y');
  });
});
